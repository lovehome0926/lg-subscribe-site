...
const App: React.FC = () => {
  ...
  const handleAiConsult = async () => {
    ...
    try {
      ...
      setAiResponse("System error: Unable to reach AI Mentor. Check your connection.");
    } finally {
          ...
          {activeTab === 'schedule' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-4">
              <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                <div className="flex items-center gap-2"><Calendar size={20} className="text-[#A50034]" /><span className="text-sm font-black text-gray-800">{selectedMonth}</span></div>
                {auth.role === 'LSM' && (
                  <button onClick={autoGenerate} className="bg-[#A50034] text-white px-5 py-2 rounded-xl text-xs font-black shadow-lg flex items-center gap-2"><Zap size={14} fill="currentColor" /> {t.generate}</button>
                )}
              </div>
              <div className="bg-white rounded-[2rem] border-2 border-slate-200 shadow-xl overflow-hidden">
                <div className="grid grid-cols-7 bg-slate-100/80 border-b-2 border-slate-200">
                  {t.weekdays.map(wd => (
                    <div key={wd} className="py-2.5 text-center text-[9px] font-black text-slate-500 uppercase border-r border-slate-200 last:border-r-0">{wd}</div>
                  ))}
                </div>
                <div className="grid grid-cols-7 bg-slate-50">
                  ...
                  {timetable.map(d => (
                    <div key={d.day} className={`min-h-[150px] border-r border-b border-slate-200 flex flex-col transition-all hover:bg-red-50/40 relative ${d.isWeekend ? 'bg-red-50/5' : 'bg-white'}`}>
                      <div className="p-1.5 flex justify-between items-center border-b border-slate-100 bg-slate-50/50">
                        <span className={`text-[10px] font-black ${d.isWeekend ? 'text-red-500' : 'text-slate-500'}`}>{d.day}</span>
                      </div>
                      <div className="flex-1 flex flex-col p-1 gap-1 border-b border-slate-100">
                        <div className="flex items-center gap-0.5 opacity-40 mb-0.5"><Sun size={8} className="text-orange-500" /><span className="text-[6px] font-black text-orange-600 uppercase">AM</span></div>
                        <div className="flex flex-col gap-0.5">
                          {d.slot1.map((name, i) => {
                            const colors = getAgentColor(name);
                            const isMe = currentUser && name === currentUser.name;
                            return (
                              <div key={i} className={`text-[8px] font-black px-1 py-0.5 rounded-[3px] truncate border shadow-sm ${colors.bg} ${colors.text} ${isMe ? 'ring-1 ring-[#A50034] border-[#A50034]' : colors.border}`}>{name}{isMe ? ' (You)' : ''}</div>
                            );
                          })}
                        </div>
                      </div>
                      <div className="flex-1 flex flex-col p-1 gap-1">
                        <div className="flex items-center gap-0.5 opacity-40 mb-0.5"><Moon size={8} className="text-indigo-500" /><span className="text-[6px] font-black text-indigo-600 uppercase">PM</span></div>
                        <div className="flex flex-col gap-0.5">
                          {d.slot2.map((name, i) => {
                            const colors = getAgentColor(name);
                            const isMe = currentUser && name === currentUser.name;
                            return (
                              <div key={i} className={`text-[8px] font-black px-1 py-0.5 rounded-[3px] truncate border shadow-sm brightness-95 ${colors.bg} ${colors.text} ${isMe ? 'ring-1 ring-[#A50034] border-[#A50034]' : colors.border}`}>{name}{isMe ? ' (You)' : ''}</div>
                            );
                          })}
                        </div>
                      </div>
          ...
          {activeTab === 'ai' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
              <div className="bg-[#A50034] p-6 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden">
                <div className="relative z-10"><h2 className="text-2xl font-black mb-1 flex items-center gap-3"><Sparkles size={28} /> {t.aiCoach}</h2><p className="text-[11px] font-medium opacity-80 uppercase tracking-widest">{t.aiInstruction}</p></div>
              </div>
              <div className="bg-white rounded-[2rem] p-6 shadow-xl border border-gray-50">
                <textarea 
                  className="w-full bg-gray-50 rounded-2xl p-5 text-sm font-semibold border-none outline-none min-h-[160px] resize-none ring-1 ring-gray-100 focus:ring-2 focus:ring-[#A50034]" 
                ...
                <button onClick={handleAiConsult} disabled={isAiLoading || !aiInput.trim()} className={`w-full mt-4 py-4 rounded-2xl flex items-center justify-center gap-3 font-black text-sm shadow-xl transition-all ${isAiLoading ? 'bg-gray-100 text-gray-400' : 'bg-[#A50034] text-white active:scale-95'}`}>
                  {isAiLoading ? <span className="animate-pulse">{t.thinking}</span> : <><Send size={18} /> {t.expertBtn}</>}
                </button>
              </div>
              {aiResponse && (
                <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-[#A50034]/10 animate-in fade-in zoom-in duration-500">
                  <div className="flex items-center justify-between mb-4">
                    <span className="font-black text-xs text-gray-800 uppercase tracking-widest">Mentor Script</span>
                    <button onClick={() => copyToClipboard(aiResponse)} className={`text-[10px] font-black px-4 py-2 rounded-full transition-all ${copyFeedback ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-[#A50034]'}`}>
                      {copyFeedback ? 'Copied' : t.copy}
                    </button>
                  </div>
          ...
          {activeTab === 'salesKit' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
              <div className="relative group">
                ...
                <input type="text" placeholder={t.searchPlaceholder} className="w-full bg-white border-none ring-1 ring-gray-100 rounded-3xl pl-14 pr-6 py-4 text-sm font-bold shadow-xl outline-none" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <div className="flex gap-2 p-1.5 bg-gray-200/50 backdrop-blur rounded-2xl">
                {(['faq', 'specs', 'scripts'] as SalesKitSubTab[]).map(tab => (
                  <button key={tab} onClick={() => setSalesKitSubTab(tab)} className={`flex-1 py-2.5 text-[11px] font-black rounded-xl uppercase transition-all ${salesKitSubTab === tab ? 'bg-white shadow-md text-[#A50034]' : 'text-gray-500'}`}>{t[tab]}</button>
                ))}
              </div>
                ...
                {salesKitSubTab === 'specs' && PRODUCT_DATA.map(p => (
                  <div key={p.id} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-2xl space-y-3">
                      <p className="text-[11px] font-bold text-gray-700">{p.dims}</p>
                      <p className="text-[11px] font-bold text-red-600 italic border-t pt-2">{p.notes}</p>
                    </div>
                  </div>
                ))}
                {salesKitSubTab === 'faq' && FAQ_DATA.map((f, i) => (
                   <div key={i} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-50">
                      <p className="text-xs font-black text-gray-800 mb-2">Q: {f.q}</p>
                      <p className="text-xs text-gray-500 font-medium leading-relaxed">A: {f.a}</p>
                   </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === 'agents' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
              {agents.filter(a => auth.role === 'LSM' || a.id === auth.agentId).map(agent => (
                <div key={agent.id} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl mb-8">
                  <div className="flex items-center gap-5 mb-8">
                    <div className={`w-16 h-16 ${getAgentColor(agent.name).bg} ${getAgentColor(agent.name).text} rounded-3xl flex items-center justify-center text-3xl font-black shadow-xl`}>{agent.name[0]}</div>
                    <div>
                      <h3 className="text-xl font-black text-gray-900">{agent.name}{currentUser && agent.id === currentUser.id ? ' (You)' : ''}</h3>
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest">{agent.type === 'FT' ? t.ft : t.pt}</span>
                    </div>
                  </div>
                  <div className="space-y-6">
                    <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest flex gap-2"><CalendarDays size={18} className="text-[#A50034]" /> Availability</p>
