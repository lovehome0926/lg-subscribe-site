...
const Navbar: React.FC<NavbarProps> = ({ currentRoute, activeAgent, isAdmin, language, setLanguage, brandingLogo }) => {
  ...
  const t = {
    ...
    cn: { home: "首页", all: "产品目录", promo: "限时优惠", join: "加入我们", partner: "认证代理人" },
    ms: { home: "Utama", all: "Katalog", promo: "Promosi", join: "SERTAI KAMI", partner: "Ejen Sah" }
  }[language];

  const scrollToSection = (id: string) => {
    setIsMenuOpen(false);
    // 强制跳转回首页，如果是从其他哈希页面点入
    if (window.location.hash.includes('admin') || window.location.hash.includes('agent-tools')) {
