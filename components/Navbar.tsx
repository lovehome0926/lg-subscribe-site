  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between w-full">
          <div className="flex items-center gap-10">
            <a href="#home" className="flex-shrink-0">
              <img src={logoSrc} className="h-8 md:h-9 w-auto object-contain" alt="LG" />
            </a>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex p-1 rounded-full bg-gray-200/20 border border-gray-200">
            ...
            <button 
              ...
              className="hidden sm:block text-[11px] font-black uppercase tracking-[0.2em] text-lg-red hover:opacity-70 border-b-2 border-lg-red/20 pb-1"
            >
              {t.join}
            </button>

            {activeAgent && (
              <div className="hidden xl:flex items-center gap-3 bg-lg-red text-white pl-1.5 pr-4 py-1.5 rounded-full shadow-lg">
            ...
            <button onClick={() => setIsMenuOpen(true)} className="lg:hidden p-2 text-gray-950 hover:bg-gray-100 rounded-full">
              <Menu size={24} />
            </button>
          </div>
        </div>
      ...
      <div className={`fixed inset-0 z-[2000] transition-all duration-500 ${isMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-[85%] bg-white shadow-3xl transition-transform duration-500 p-12 flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
           <div className="flex justify-between items-center mb-16">
             <img src={logoSrc} className="h-7" alt="LG" />
             <button onClick={() => setIsMenuOpen(false)} className="p-3 bg-gray-50 rounded-full"><X size={24}/></button>
           </div>
           <nav className="flex flex-col gap-8">
             {navLinks.map(link => (
               <button key={link.id} onClick={link.action} className="text-left text-3xl font-black uppercase tracking-tighter text-gray-950">
                 {link.label}
               </button>
             ))}
             <button onClick={() => { document.getElementById('join-us-section')?.scrollIntoView({ behavior: 'smooth' }); setIsMenuOpen(false); }} className="text-left text-3xl font-black uppercase tracking-tighter text-lg-red">
               {t.join}
             </button>
           </nav>
        </div>
