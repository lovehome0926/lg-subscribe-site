...
const Footer: React.FC<FooterProps> = ({ brandingLogo, siteSettings, language, activeAgent }) => {
  const t = {
    ...
    ms: { 
      ...
      titleLine1: "KEHIDUPAN STABIL.",
      titleLine2: "KAMI BERI LEBIH.",
      explore: "TEROKAI", 
      visit: "LOKASI KAMI", 
      contact: "HUBUNGI", 
      apply: "INFO LANJUT DI CHAT", 
      recruitment_msg: "Hai! Saya berminat dengan Program Rakan LG 2026. Boleh jelaskan lebih lanjut tentang komisyen 13%?", 
  ...
  return (
    <footer id="footer-section" className="bg-[#05090f] text-white pt-20 pb-20">
      <div className="container mx-auto px-6 max-w-7xl">
        
        {/* EXACT REFERENCE IMAGE REPLICA - PARTNER REWARDS CARD */}
        <section id="join-us-section" className="bg-white rounded-[40px] md:rounded-[60px] p-8 md:p-16 lg:p-24 text-gray-950 mb-40 relative overflow-hidden shadow-3xl">
           ...
           <div className="relative z-10 space-y-12 md:space-y-16">
              ...
              <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-start">
                 ...
                 {/* Column 2: Incentive Highlights */}
                 <div className="lg:col-span-5 space-y-6 md:space-y-8">
                    <span className="text-[10px] font-black uppercase text-lg-red tracking-[0.4em] block">{t.benefits}</span>
                    <div className="flex flex-col gap-3">
                       {highlights.map((b, i) => (
                          <div key={i} className="flex items-center gap-4 bg-white px-5 py-4 rounded-2xl text-[10px] md:text-[11px] font-black uppercase tracking-widest border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl transition-all">
                 ...
                 <div className="lg:col-span-3 flex justify-center lg:justify-end lg:pt-8">
                    <a 
                      href={recruitmentWa} 
                      target="_blank" 
                      className="bg-lg-red text-white px-10 py-6 md:py-8 rounded-[35px] font-black uppercase tracking-[0.2em] text-[12px] md:text-[14px] shadow-[0_25px_60px_rgba(230,0,68,0.3)] hover:bg-gray-950 hover:scale-105 active:scale-95 transition-all block text-center min-w-[240px]"
                    >
                      {t.apply}
                    </a>
                 </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-16 border-t border-white/5 pt-20">
          <div className="md:col-span-3 space-y-6">
            <img 
              ...
              alt="LG" 
            />
            <p className="text-gray-500 text-xs font-medium uppercase tracking-widest opacity-60">Digital Partner Showroom.</p>
          </div>
          <div className="md:col-span-6 grid grid-cols-2 gap-8">
            <div>
              <h4 className="text-[10px] font-black uppercase text-lg-red tracking-[0.4em] mb-6">{t.visit}</h4>
              <div className="space-y-4">
                 {siteSettings.stores.slice(0, 2).map(s => (
                   <p key={s.id} className="text-[11px] font-bold text-gray-400 uppercase tracking-tight">{s.name}</p>
                 ))}
              </div>
            </div>
            <div>
               <h4 className="text-[10px] font-black uppercase text-gray-700 tracking-[0.4em] mb-6">{t.contact}</h4>
               <p className="text-[11px] font-black break-all text-white/40">{siteSettings.officeEmail}</p>
            </div>
          </div>
          <div className="md:col-span-3 flex gap-4 justify-end items-start">
             {['fb', 'ig', 'tiktok'].map(k => (
                <div key={k} className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-[8px] font-black uppercase opacity-40 hover:opacity-100 hover:border-lg-red transition-all cursor-pointer">{k}</div>
             ))}
          </div>
        </div>
      </div>
